...
import type { ExtraUi } from "./extra";
...
/** What components consume: the per-locale dictionary plus the `extra` block
 *  that `getDictionary` merges in. Locale files declare `BaseUiDictionary`. */
export type UiDictionary = BaseUiDictionary & {
  /** Strings added alongside the Systems/Saved/Notes/Lesson features. */
  app: ExtraUi;
};

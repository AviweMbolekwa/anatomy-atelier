...
type ViewerCallbacks = {
  ...
  /** A model failed to load — the UI shows an error with a retry. */
  onLoadError?: (error: Error) => void;
  /** GLB load profiling: how long a switch took, and whether it was cached. */
  onLoadTiming?: (timing: { url: string; ms: number; cached: boolean }) => void;
};
...
export class AnatomyViewer {
  ...
  async setOrgan(modelUrl: string, hotspots: Hotspot[], accent: string) {
    ...
    const wasCached = this.assets.isCached(modelUrl);
    try {
      ...
      this.callbacks.onLoadError?.(error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
    // Cached vs cold loads have very different profiles; reporting both lets
    // the caller see whether prefetching is actually paying off.
    this.callbacks.onLoadTiming?.({
      url: modelUrl,
  ...
  private onKeyDown = (event: KeyboardEvent) => {
    ...
    // Up/Down step through the labelled structures; Left/Right still spin the
    // specimen, so both the model and its labels are keyboard-reachable.
  ...
  /**
   * Returns the viewer to its opening state. "Reset" previously moved the
   * camera and cleared the selection but left isolate, cross-section and
   * wireframe engaged, so the button visibly did not undo the tools it sat
   * next to. Everything a tool can change is now reverted here, and the
   * caller is told the resulting tool state so its UI can follow.
   */
  reset(): ResetState {
    this.select(null);
    this.hotspots.clearFlash();

    // Camera, target and organ orientation.
    ...
    if (this.organ) {
      this.tween(this.organ.pivot.rotation, { x: 0.05, y: -0.28, z: 0, duration: 0.8, ease: "power3.out" });
      this.tween(this.organ.pivot.position, { x: 0, y: 0, z: 0, duration: 0.8, ease: "power3.out" });
      this.tween(this.organ.pivot.scale, { x: 1, y: 1, z: 1, duration: 0.8, ease: "power3.out" });
    }

    // Isolate: bring the plinth and contact shadow back.
    if (this.isolated) {
      this.isolated = false;
    ...
    // Cross-section: drop the clipping plane.
    ...
    // Layers: clear wireframe, and restore any opacity a fade left behind.
    if (this.organ) {
      this.fadeTween?.kill();
      this.fadeTween = null;
      this.setDepthPrepass(this.organ, false);
    ...
    // Auto-rotation returns to the default "on", and any interaction hold that
    // was suppressing it is released.
    this.autoRotateWanted = true;
    this.interactionUntil = 0;

    this.busy(0.9);
    this.dirty = true;
    return { autoRotate: true, isolated: false, crossSection: false, layers: false };
  }

  /**
   * Moves the selection to the next/previous structure, wrapping around.
   * Backs the keyboard path through the hotspot list so the dots are reachable
   * without a pointer. Returns the newly selected hotspot, if any.
   */
  cycleHotspot(direction: 1 | -1): Hotspot | null {
    const markers = this.hotspots.list;
    if (!markers.length) return null;
    const currentIndex = markers.findIndex((marker) => marker.hotspot.id === this.selectedId);
    const nextIndex = currentIndex === -1
      ? (direction === 1 ? 0 : markers.length - 1)
      : (currentIndex + direction + markers.length) % markers.length;
    const next = markers[nextIndex].hotspot;
    this.select(next.id);
    return next;
  }

  /** Selects a structure by id — used by the keyboard-accessible index list. */
